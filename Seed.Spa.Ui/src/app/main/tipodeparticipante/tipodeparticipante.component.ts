...
@Component({
    selector: 'app-tipodeparticipante',
    templateUrl: './tipodeparticipante.component.html',
    styleUrls: ['./tipodeparticipante.component.css'],
})
export class TipoDeParticipanteComponent extends ComponentBase implements OnInit, OnDestroy {
    ...
    constructor(private tipoDeParticipanteService: TipoDeParticipanteService, private router: Router, private ref: ChangeDetectorRef) {
    ...
    ngOnInit() {
        ...
        this.tipoDeParticipanteService.OnHide(this.saveModal, this.editModal, () => { this.hideComponents() });
    ...
    public onFilter(modelFilter: any) {
        modelFilter.queryOptimizerBehavior = "GRID_TipoDeParticipante".toUpperCase();
        this.tipoDeParticipanteService.get(modelFilter).subscribe((result) => {
    ...
    public onExport() {
        this.tipoDeParticipanteService.export(Object.assign(this.vm.modelFilter, { AttributeBehavior: "exportar" })).subscribe((result) => {
            ...
            a.href = downloadUrl;
            a.download = "TipoDeParticipante.xlsx";
            a.click();
    ...
    public onCreate() {
        ...
        this.navigateStrategy(this.vm, this.saveModal, this.router, "/tipodeparticipante/create");
    }
    ...
    public onEdit(model: any) {
        ...
        if (model)  {
            newModel = { id: model.tipoDeParticipanteId }
        }

        if (!this.vm.navigationModal) {
            this.navigateStrategy(this.vm, this.editModal, this.router, "/tipodeparticipante/edit/" + newModel.id);
        }
        else {
            this.tipoDeParticipanteService.get(newModel).subscribe((result) => {
    ...
    public onDetails(model: any) {
        ...
        if (!this.vm.navigationModal) {
            this.navigateStrategy(this.vm, this.editModal, this.router, "/tipodeparticipante/details/" + newModel.id);
        }
        else {
            this.tipoDeParticipanteService.get(newModel).subscribe((result) => {
    ...
    public onPrint(model: any) {
        this.router.navigate(['/tipodeparticipante/print', model.tipoDeParticipanteId]);
    }
    ...
    public onDeleteConfimation(model: any) {
        ...
        var conf = GlobalService.operationExecutedParameters(
            ...
            () => {
                ...
                this.tipoDeParticipanteService.delete(newModel).subscribe((result) => {
                    this.vm.filterResult = this.vm.filterResult.filter(function (model) {
                        return  model.tipoDeParticipanteId != result.data.tipoDeParticipanteId;
                    });
    ...
    public onPageChanged(pageConfig: any) {
        ...
        let modelFilter = this.tipoDeParticipanteService.pagingConfig(this.vm.modelFilter, pageConfig);
        this.tipoDeParticipanteService.get(modelFilter).subscribe((result) => {
            this.vm.filterResult = result.dataList;
            this.vm.summary = result.summary;
        });
    ...
    public onOrderBy(order: any) {
        ...
        let modelFilter = this.tipoDeParticipanteService.orderByConfig(this.vm.modelFilter, order);
        this.tipoDeParticipanteService.get(modelFilter).subscribe((result) => {
            this.vm.filterResult = result.dataList;
            this.vm.summary = result.summary;
        });
