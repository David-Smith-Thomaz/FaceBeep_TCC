...
@Injectable()
export class StatusUserService extends ServiceBase {

    private _form : FormGroup;

    constructor(private api: ApiService<any>,private serviceFields: StatusUserServiceFields, private globalServiceCulture: GlobalServiceCulture, private mainService: MainService) {
    ...
    getInfosTranslated(culture: string) {
        return this.globalServiceCulture.getInfosTranslatedStrategy('StatusUser', culture, this.getInfos(), []);
    }

    get(filters?: any): Observable<any> {
        return this.api.setResource('StatusUser').get(filters);
    }

    getDataCustom(filters?: any): Observable<any> {
        return this.api.setResource('StatusUser').getDataCustom(filters);
    }

    getDataListCustom(filters?: any): Observable<any> {
        return this.api.setResource('StatusUser').getDataListCustom(filters);
    }

    save(model: any): Observable<any> {

        if (model.statusUserId) {
            return this.api.setResource('StatusUser').put(model);
        }

        return this.api.setResource('StatusUser').post(model);
    }

    delete(model: any): Observable<any> {
        return this.api.setResource('StatusUser').delete(model);
    }
    
    export(filters?: any): Observable<any> {
        return this.api.setResource('StatusUser').export(filters);
    }
}
