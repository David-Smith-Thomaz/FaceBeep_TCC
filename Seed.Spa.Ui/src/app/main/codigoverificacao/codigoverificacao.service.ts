...
@Injectable()
export class CodigoVerificacaoService extends ServiceBase {

    private _form : FormGroup;

    constructor(private api: ApiService<any>,private serviceFields: CodigoVerificacaoServiceFields, private globalServiceCulture: GlobalServiceCulture, private mainService: MainService) {
    ...
    getInfosTranslated(culture: string) {
        return this.globalServiceCulture.getInfosTranslatedStrategy('CodigoVerificacao', culture, this.getInfos(), []);
    }

    get(filters?: any): Observable<any> {
        return this.api.setResource('CodigoVerificacao').get(filters);
    }

    getDataCustom(filters?: any): Observable<any> {
        return this.api.setResource('CodigoVerificacao').getDataCustom(filters);
    }

    getDataListCustom(filters?: any): Observable<any> {
        return this.api.setResource('CodigoVerificacao').getDataListCustom(filters);
    }

    save(model: any): Observable<any> {

        if (model.codigoVerificacaoId) {
            return this.api.setResource('CodigoVerificacao').put(model);
        }

        return this.api.setResource('CodigoVerificacao').post(model);
    }

    delete(model: any): Observable<any> {
        return this.api.setResource('CodigoVerificacao').delete(model);
    }
    
    export(filters?: any): Observable<any> {
        return this.api.setResource('CodigoVerificacao').export(filters);
    }
}
