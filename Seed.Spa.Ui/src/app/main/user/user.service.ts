...
export class UserService extends ServiceBase {
    ...
    constructor(private api: ApiService<any>,private serviceFields: UserServiceFields, private globalServiceCulture: GlobalServiceCulture, private mainService: MainService) {
    ...
    getInfosTranslated(culture: string) {
        return this.globalServiceCulture.getInfosTranslatedStrategy('User', culture, this.getInfos(), []);
    }

    get(filters?: any): Observable<any> {
        return this.api.setResource('User').get(filters);
    }
    ...
    getDataListCustom(filters?: any): Observable<any> {
        return this.api.setResource('User').getDataListCustom(filters);
    }

    save(model: any): Observable<any> {

        if (model.userId) {
            return this.api.setResource('User').put(model);
        }

        return this.api.setResource('User').post(model);
    }

    delete(model: any): Observable<any> {
        return this.api.setResource('User').delete(model);
    }
    
    export(filters?: any): Observable<any> {
        return this.api.setResource('User').export(filters);
    }
}
