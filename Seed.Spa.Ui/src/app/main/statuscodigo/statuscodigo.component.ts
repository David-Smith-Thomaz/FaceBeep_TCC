...
@Component({
    selector: 'app-statuscodigo',
    templateUrl: './statuscodigo.component.html',
    styleUrls: ['./statuscodigo.component.css'],
})
export class StatusCodigoComponent extends ComponentBase implements OnInit, OnDestroy {
    ...
    constructor(private statusCodigoService: StatusCodigoService, private router: Router, private ref: ChangeDetectorRef) {
    ...
    ngOnInit() {
        ...
        this.statusCodigoService.OnHide(this.saveModal, this.editModal, () => { this.hideComponents() });
    ...
    public onFilter(modelFilter: any) {
        modelFilter.queryOptimizerBehavior = "GRID_StatusCodigo".toUpperCase();
        this.statusCodigoService.get(modelFilter).subscribe((result) => {
    ...
    public onExport() {
        this.statusCodigoService.export(Object.assign(this.vm.modelFilter, { AttributeBehavior: "exportar" })).subscribe((result) => {
    ...
    public onCreate() {
        ...
        this.navigateStrategy(this.vm, this.saveModal, this.router, "/statuscodigo/create");
    }
    ...
    public onEdit(model: any) {
        ...
        if (model)  {
            newModel = { id: model.statusCodigoId }
        }

        if (!this.vm.navigationModal) {
            this.navigateStrategy(this.vm, this.editModal, this.router, "/statuscodigo/edit/" + newModel.id);
        }
        else {
            this.statusCodigoService.get(newModel).subscribe((result) => {
    ...
    public onDetails(model: any) {
        ...
        if (!this.vm.navigationModal) {
            this.navigateStrategy(this.vm, this.editModal, this.router, "/statuscodigo/details/" + newModel.id);
        }
        else {
            this.statusCodigoService.get(newModel).subscribe((result) => {
    ...
    public onPrint(model: any) {
        this.router.navigate(['/statuscodigo/print', model.statusCodigoId]);
    }
    ...
    public onDeleteConfimation(model: any) {
        ...
        var conf = GlobalService.operationExecutedParameters(
            ...
            () => {
                ...
                this.statusCodigoService.delete(newModel).subscribe((result) => {
                    this.vm.filterResult = this.vm.filterResult.filter(function (model) {
                        return  model.statusCodigoId != result.data.statusCodigoId;
                    });
    ...
    public onPageChanged(pageConfig: any) {
        ...
        let modelFilter = this.statusCodigoService.pagingConfig(this.vm.modelFilter, pageConfig);
        this.statusCodigoService.get(modelFilter).subscribe((result) => {
            this.vm.filterResult = result.dataList;
            this.vm.summary = result.summary;
        });
    ...
    public onOrderBy(order: any) {
        ...
        let modelFilter = this.statusCodigoService.orderByConfig(this.vm.modelFilter, order);
        this.statusCodigoService.get(modelFilter).subscribe((result) => {
            this.vm.filterResult = result.dataList;
            this.vm.summary = result.summary;
        });
