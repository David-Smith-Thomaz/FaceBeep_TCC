...
@Component({
    selector: 'app-turmaparticipante',
    templateUrl: './turmaparticipante.component.html',
    styleUrls: ['./turmaparticipante.component.css'],
})
export class TurmaParticipanteComponent extends ComponentBase implements OnInit, OnDestroy {
    ...
    constructor(private turmaParticipanteService: TurmaParticipanteService, private router: Router, private ref: ChangeDetectorRef) {
    ...
    ngOnInit() {
        ...
        this.turmaParticipanteService.OnHide(this.saveModal, this.editModal, () => { this.hideComponents() });
    ...
    public onFilter(modelFilter: any) {
        modelFilter.queryOptimizerBehavior = "GRID_TurmaParticipante".toUpperCase();
        this.turmaParticipanteService.get(modelFilter).subscribe((result) => {
    ...
    public onExport() {
        this.turmaParticipanteService.export(Object.assign(this.vm.modelFilter, { AttributeBehavior: "exportar" })).subscribe((result) => {
            ...
            a.href = downloadUrl;
            a.download = "TurmaParticipante.xlsx";
            a.click();
    ...
    public onCreate() {
        ...
        this.navigateStrategy(this.vm, this.saveModal, this.router, "/turmaparticipante/create");
    }
    ...
    public onEdit(model: any) {
        ...
        if (model)  {
            newModel = { id: model.turmaParticipanteId }
        }

        if (!this.vm.navigationModal) {
            this.navigateStrategy(this.vm, this.editModal, this.router, "/turmaparticipante/edit/" + newModel.id);
        }
        else {
            this.turmaParticipanteService.get(newModel).subscribe((result) => {
    ...
    public onDetails(model: any) {
        ...
        if (!this.vm.navigationModal) {
            this.navigateStrategy(this.vm, this.editModal, this.router, "/turmaparticipante/details/" + newModel.id);
        }
        else {
            this.turmaParticipanteService.get(newModel).subscribe((result) => {
    ...
    public onPrint(model: any) {
        this.router.navigate(['/turmaparticipante/print', model.turmaParticipanteId]);
    }
    ...
    public onDeleteConfimation(model: any) {
        ...
        var conf = GlobalService.operationExecutedParameters(
            ...
            () => {
                ...
                this.turmaParticipanteService.delete(newModel).subscribe((result) => {
                    this.vm.filterResult = this.vm.filterResult.filter(function (model) {
                        return  model.turmaParticipanteId != result.data.turmaParticipanteId;
                    });
    ...
    public onPageChanged(pageConfig: any) {
        ...
        let modelFilter = this.turmaParticipanteService.pagingConfig(this.vm.modelFilter, pageConfig);
        this.turmaParticipanteService.get(modelFilter).subscribe((result) => {
            this.vm.filterResult = result.dataList;
            this.vm.summary = result.summary;
        });
    ...
    public onOrderBy(order: any) {
        ...
        let modelFilter = this.turmaParticipanteService.orderByConfig(this.vm.modelFilter, order);
        this.turmaParticipanteService.get(modelFilter).subscribe((result) => {
            this.vm.filterResult = result.dataList;
            this.vm.summary = result.summary;
        });
