...
export class TypeUserComponent extends ComponentBase implements OnInit, OnDestroy {
    ...
    constructor(private typeUserService: TypeUserService, private router: Router, private ref: ChangeDetectorRef) {
    ...
    ngOnInit() {
        ...
        this.typeUserService.OnHide(this.saveModal, this.editModal, () => { this.hideComponents() });
    ...
    public onFilter(modelFilter: any) {
        modelFilter.queryOptimizerBehavior = "GRID_TypeUser".toUpperCase();
        this.typeUserService.get(modelFilter).subscribe((result) => {
    ...
    public onExport() {
        this.typeUserService.export(Object.assign(this.vm.modelFilter, { AttributeBehavior: "exportar" })).subscribe((result) => {
    ...
    public onCreate() {
        ...
        this.navigateStrategy(this.vm, this.saveModal, this.router, "/typeuser/create");
    }
    ...
    public onEdit(model: any) {
        ...
        if (model)  {
            newModel = { id: model.typeUserId }
        }

        if (!this.vm.navigationModal) {
            this.navigateStrategy(this.vm, this.editModal, this.router, "/typeuser/edit/" + newModel.id);
        }
        else {
            this.typeUserService.get(newModel).subscribe((result) => {
    ...
    public onDetails(model: any) {
        ...
        if (!this.vm.navigationModal) {
            this.navigateStrategy(this.vm, this.editModal, this.router, "/typeuser/details/" + newModel.id);
        }
        else {
            this.typeUserService.get(newModel).subscribe((result) => {
    ...
    public onPrint(model: any) {
        this.router.navigate(['/typeuser/print', model.typeUserId]);
    }
    ...
    public onDeleteConfimation(model: any) {
        ...
        var conf = GlobalService.operationExecutedParameters(
            ...
            () => {
                ...
                this.typeUserService.delete(newModel).subscribe((result) => {
                    this.vm.filterResult = this.vm.filterResult.filter(function (model) {
                        return  model.typeUserId != result.data.typeUserId;
                    });
    ...
    public onPageChanged(pageConfig: any) {
        ...
        let modelFilter = this.typeUserService.pagingConfig(this.vm.modelFilter, pageConfig);
        this.typeUserService.get(modelFilter).subscribe((result) => {
            this.vm.filterResult = result.dataList;
            this.vm.summary = result.summary;
        });
    ...
    public onOrderBy(order: any) {
        ...
        let modelFilter = this.typeUserService.orderByConfig(this.vm.modelFilter, order);
        this.typeUserService.get(modelFilter).subscribe((result) => {
            this.vm.filterResult = result.dataList;
            this.vm.summary = result.summary;
        });
