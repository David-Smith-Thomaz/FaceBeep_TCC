...
export class ParticipanteComponent extends ComponentBase implements OnInit, OnDestroy {
    ...
    constructor(private participanteService: ParticipanteService, private router: Router, private ref: ChangeDetectorRef) {
    ...
    ngOnInit() {
        ...
        this.participanteService.OnHide(this.saveModal, this.editModal, () => { this.hideComponents() });
    ...
    public onFilter(modelFilter: any) {
        modelFilter.queryOptimizerBehavior = "GRID_Participante".toUpperCase();
        this.participanteService.get(modelFilter).subscribe((result) => {
    ...
    public onExport() {
        this.participanteService.export(Object.assign(this.vm.modelFilter, { AttributeBehavior: "exportar" })).subscribe((result) => {
    ...
    public onCreate() {
        ...
        this.navigateStrategy(this.vm, this.saveModal, this.router, "/participante/create");
    }
    ...
    public onEdit(model: any) {
        ...
        if (model)  {
            newModel = { id: model.participanteId }
        }

        if (!this.vm.navigationModal) {
            this.navigateStrategy(this.vm, this.editModal, this.router, "/participante/edit/" + newModel.id);
        }
        else {
            this.participanteService.get(newModel).subscribe((result) => {
    ...
    public onDetails(model: any) {
        ...
        if (!this.vm.navigationModal) {
            this.navigateStrategy(this.vm, this.editModal, this.router, "/participante/details/" + newModel.id);
        }
        else {
            this.participanteService.get(newModel).subscribe((result) => {
    ...
    public onPrint(model: any) {
        this.router.navigate(['/participante/print', model.participanteId]);
    }
    ...
    public onDeleteConfimation(model: any) {
        ...
        var conf = GlobalService.operationExecutedParameters(
            ...
            () => {
                ...
                this.participanteService.delete(newModel).subscribe((result) => {
                    this.vm.filterResult = this.vm.filterResult.filter(function (model) {
                        return  model.participanteId != result.data.participanteId;
                    });
    ...
    public onPageChanged(pageConfig: any) {
        ...
        let modelFilter = this.participanteService.pagingConfig(this.vm.modelFilter, pageConfig);
        this.participanteService.get(modelFilter).subscribe((result) => {
            this.vm.filterResult = result.dataList;
            this.vm.summary = result.summary;
        });
    ...
    public onOrderBy(order: any) {
        ...
        let modelFilter = this.participanteService.orderByConfig(this.vm.modelFilter, order);
        this.participanteService.get(modelFilter).subscribe((result) => {
            this.vm.filterResult = result.dataList;
            this.vm.summary = result.summary;
        });
