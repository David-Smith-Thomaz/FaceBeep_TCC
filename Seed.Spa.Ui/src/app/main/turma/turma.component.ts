...
export class TurmaComponent extends ComponentBase implements OnInit, OnDestroy {
    ...
    constructor(private turmaService: TurmaService, private router: Router, private ref: ChangeDetectorRef) {
    ...
    ngOnInit() {
        ...
        this.turmaService.OnHide(this.saveModal, this.editModal, () => { this.hideComponents() });
    ...
    public onFilter(modelFilter: any) {
        modelFilter.queryOptimizerBehavior = "GRID_Turma".toUpperCase();
        this.turmaService.get(modelFilter).subscribe((result) => {
    ...
    public onExport() {
        this.turmaService.export(Object.assign(this.vm.modelFilter, { AttributeBehavior: "exportar" })).subscribe((result) => {
    ...
    public onCreate() {
        ...
        this.navigateStrategy(this.vm, this.saveModal, this.router, "/turma/create");
    }
    ...
    public onEdit(model: any) {
        ...
        if (model)  {
            newModel = { id: model.turmaId }
        }

        if (!this.vm.navigationModal) {
            this.navigateStrategy(this.vm, this.editModal, this.router, "/turma/edit/" + newModel.id);
        }
        else {
            this.turmaService.get(newModel).subscribe((result) => {
    ...
    public onDetails(model: any) {
        ...
        if (!this.vm.navigationModal) {
            this.navigateStrategy(this.vm, this.editModal, this.router, "/turma/details/" + newModel.id);
        }
        else {
            this.turmaService.get(newModel).subscribe((result) => {
    ...
    public onPrint(model: any) {
        this.router.navigate(['/turma/print', model.turmaId]);
    }
    ...
    public onDeleteConfimation(model: any) {
        ...
        var conf = GlobalService.operationExecutedParameters(
            ...
            () => {
                ...
                this.turmaService.delete(newModel).subscribe((result) => {
                    this.vm.filterResult = this.vm.filterResult.filter(function (model) {
                        return  model.turmaId != result.data.turmaId;
                    });
    ...
    public onPageChanged(pageConfig: any) {
        ...
        let modelFilter = this.turmaService.pagingConfig(this.vm.modelFilter, pageConfig);
        this.turmaService.get(modelFilter).subscribe((result) => {
            this.vm.filterResult = result.dataList;
            this.vm.summary = result.summary;
        });
    ...
    public onOrderBy(order: any) {
        ...
        let modelFilter = this.turmaService.orderByConfig(this.vm.modelFilter, order);
        this.turmaService.get(modelFilter).subscribe((result) => {
            this.vm.filterResult = result.dataList;
            this.vm.summary = result.summary;
        });
