...
{
	[Authorize]
    [Route("api/statususer/more")]
    public class StatusUserMoreController : Controller
    {

        private readonly IStatusUserRepository _rep;
        private readonly IStatusUserApplicationService _app;
		private readonly ILogger _logger;
		private readonly EnviromentInfo _env;
		private readonly CurrentUser _user;

        public StatusUserMoreController(IStatusUserRepository rep, IStatusUserApplicationService app, ILoggerFactory logger, EnviromentInfo env,CurrentUser user)
        {
            this._rep = rep;
            this._app = app;
			this._logger = logger.CreateLogger<StatusUserMoreController>();
			this._env = env;
			this._user = user;
        }
        ...
        public async Task<IActionResult> Get([FromQuery]StatusUserFilter filters)
        {
            ...
            try
            {
				...
				if (!this._user.GetClaims().GetTools().VerifyClaimsCanRead("StatusUser"))
                {
				...
				if (filters.FilterBehavior == FilterBehavior.Export)
                {
                    ...
                    var file = export.ExportFile(this.Response, searchResult, "StatusUser", this._env.RootPath);
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody]IEnumerable<StatusUserDtoSpecialized> dtos)
        {
            ...
            {
                var responseEx = result.ReturnCustomException(ex,"Seed - StatusUser", dtos, new ErrorMapCustom());
				return responseEx;
            }
		...
		[Authorize(Policy = "CanWrite")]
		[HttpPut]
        public async Task<IActionResult> Put([FromBody]IEnumerable<StatusUserDtoSpecialized> dtos)
        {
            var result = new HttpResult<StatusUserDto>(this._logger);
            try
            {
