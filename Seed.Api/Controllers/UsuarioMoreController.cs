...
namespace Seed.Api.Controllers
{
    ...
    public class UsuarioMoreController : Controller
    {
        ...
        public UsuarioMoreController(IUsuarioRepository rep, IUsuarioApplicationService app, ILoggerFactory logger, EnviromentInfo env,CurrentUser user)
        {
            this._rep = rep;
            this._app = app;
        ...
        public async Task<IActionResult> Get([FromQuery]UsuarioFilter filters)
        {
            ...
            try
            {
				...
				if (!this._user.GetClaims().GetTools().VerifyClaimsCanRead("Usuario"))
                {
				...
				if (filters.FilterBehavior == FilterBehavior.Export)
                {
                    ...
                    var file = export.ExportFile(this.Response, searchResult, "Usuario", this._env.RootPath);
        ...
        public async Task<IActionResult> Post([FromBody]IEnumerable<UsuarioDtoSpecialized> dtos)
        {
            ...
            {
                var responseEx = result.ReturnCustomException(ex,"Seed - Usuario", dtos, new ErrorMapCustom());
				return responseEx;
            }
