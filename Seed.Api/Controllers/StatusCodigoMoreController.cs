...
{
	[Authorize]
    [Route("api/statuscodigo/more")]
    public class StatusCodigoMoreController : Controller
    {

        private readonly IStatusCodigoRepository _rep;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody]IEnumerable<StatusCodigoDtoSpecialized> dtos)
        {
            ...
            {
                var responseEx = result.ReturnCustomException(ex,"Seed - StatusCodigo", dtos, new ErrorMapCustom());
				return responseEx;
            }
		...
		[Authorize(Policy = "CanWrite")]
		[HttpPut]
        public async Task<IActionResult> Put([FromBody]IEnumerable<StatusCodigoDtoSpecialized> dtos)
        {
            var result = new HttpResult<StatusCodigoDto>(this._logger);
            try
            {
