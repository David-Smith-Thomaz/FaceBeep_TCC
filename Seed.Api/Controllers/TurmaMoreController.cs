...
{
	[Authorize]
    [Route("api/turma/more")]
    public class TurmaMoreController : Controller
    {

        private readonly ITurmaRepository _rep;
        private readonly ITurmaApplicationService _app;
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery]TurmaFilter filters)
        {
            ...
            try
            {
				...
				if (!this._user.GetClaims().GetTools().VerifyClaimsCanRead("Turma"))
                {
				...
				if (filters.FilterBehavior == FilterBehavior.Export)
                {
                    ...
                    var file = export.ExportFile(this.Response, searchResult, "Turma", this._env.RootPath);
            ...
            {
                var responseEx = result.ReturnCustomException(ex,"Seed - Turma", filters, new ErrorMapCustom());
				return responseEx;
            }
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody]IEnumerable<TurmaDtoSpecialized> dtos)
        {
            ...
            {
                var responseEx = result.ReturnCustomException(ex,"Seed - Turma", dtos, new ErrorMapCustom());
				return responseEx;
            }
		...
		[Authorize(Policy = "CanWrite")]
		[HttpPut]
        public async Task<IActionResult> Put([FromBody]IEnumerable<TurmaDtoSpecialized> dtos)
        {
            var result = new HttpResult<TurmaDto>(this._logger);
            try
            {
