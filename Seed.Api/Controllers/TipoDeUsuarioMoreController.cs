...
{
	[Authorize]
    [Route("api/tipodeusuario/more")]
    public class TipoDeUsuarioMoreController : Controller
    {

        private readonly ITipoDeUsuarioRepository _rep;
        private readonly ITipoDeUsuarioApplicationService _app;
		private readonly ILogger _logger;
		private readonly EnviromentInfo _env;
		private readonly CurrentUser _user;

        public TipoDeUsuarioMoreController(ITipoDeUsuarioRepository rep, ITipoDeUsuarioApplicationService app, ILoggerFactory logger, EnviromentInfo env,CurrentUser user)
        {
            this._rep = rep;
            this._app = app;
			this._logger = logger.CreateLogger<TipoDeUsuarioMoreController>();
			this._env = env;
			this._user = user;
        }
        ...
        public async Task<IActionResult> Get([FromQuery]TipoDeUsuarioFilter filters)
        {
            ...
            try
            {
				...
				if (!this._user.GetClaims().GetTools().VerifyClaimsCanRead("TipoDeUsuario"))
                {
				...
				if (filters.FilterBehavior == FilterBehavior.Export)
                {
                    ...
                    var file = export.ExportFile(this.Response, searchResult, "TipoDeUsuario", this._env.RootPath);
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody]IEnumerable<TipoDeUsuarioDtoSpecialized> dtos)
        {
            ...
            {
                var responseEx = result.ReturnCustomException(ex,"Seed - TipoDeUsuario", dtos, new ErrorMapCustom());
				return responseEx;
            }
		...
		[Authorize(Policy = "CanWrite")]
		[HttpPut]
        public async Task<IActionResult> Put([FromBody]IEnumerable<TipoDeUsuarioDtoSpecialized> dtos)
        {
            var result = new HttpResult<TipoDeUsuarioDto>(this._logger);
            try
            {
