...
{
	[Authorize]
    [Route("api/typeuser/more")]
    public class TypeUserMoreController : Controller
    {

        private readonly ITypeUserRepository _rep;
        ...
        public async Task<IActionResult> Get([FromQuery]TypeUserFilter filters)
        {
            ...
            try
            {
				...
				if (!this._user.GetClaims().GetTools().VerifyClaimsCanRead("TypeUser"))
                {
				...
				if (filters.FilterBehavior == FilterBehavior.Export)
                {
                    ...
                    var file = export.ExportFile(this.Response, searchResult, "TypeUser", this._env.RootPath);
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody]IEnumerable<TypeUserDtoSpecialized> dtos)
        {
            ...
            {
                var responseEx = result.ReturnCustomException(ex,"Seed - TypeUser", dtos, new ErrorMapCustom());
				return responseEx;
            }
		...
		[Authorize(Policy = "CanWrite")]
		[HttpPut]
        public async Task<IActionResult> Put([FromBody]IEnumerable<TypeUserDtoSpecialized> dtos)
        {
            var result = new HttpResult<TypeUserDto>(this._logger);
            try
            {
